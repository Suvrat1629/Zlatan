...
class RoadGraph(
    ...
    /** Geographic bearing (deg, 0 = north, clockwise) of the candidate's edge. Direction along
     *  the edge is arbitrary (roads are stored undirected here), so callers should treat this
     *  as an axis, not a heading. Frame rotation between the graph anchor and a nearby query
     *  point is negligible at city scale, so this is safe to hand to a filter in its own frame. */
    fun bearingDegOf(c: Candidate): Double {
        val dEast = c.edge.bx - c.edge.ax
        val dNorth = c.edge.by - c.edge.ay
        return Math.toDegrees(kotlin.math.atan2(dEast, dNorth))
    }
